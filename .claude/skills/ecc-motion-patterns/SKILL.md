...
description: Production-ready animation patterns for React / Next.js — button, modal, toast, stagger, page transitions, exit animations, scroll, and layout — built on ecc-motion-foundations tokens and springs.
version: 1.0
...
Every pattern here is built on `ecc-motion-foundations` tokens and springs.
...
- Every pattern imports from `ecc-motion-foundations`. No raw numbers.
...
8. **All token values are imported from `ecc-motion-foundations`.** No inline numbers.
...
the accessibility hook from `ecc-motion-foundations`:
...
- Token and spring definitions → see `ecc-motion-foundations`
- Drag interactions, swipe gestures, reorderable lists → see `ecc-motion-advanced`
- Text animations (word/character reveal, counters) → see `ecc-motion-advanced`
- SVG path drawing or morphing → see `ecc-motion-advanced`
- Custom animation hooks → see `ecc-motion-advanced`
...
- **`ecc-motion-foundations`** — defines all tokens, springs, the `useSafeMotion` hook, and SSR guards that every pattern here imports. Must be set up first.
- **`ecc-motion-advanced`** — extends these patterns with drag, gestures, SVG, text, custom hooks, and imperative sequencing. Does not redefine any patterns from this skill.
