...
function makeHarness(): Harness {
  ...
  const service = new ProfileService({ users, profiles, snsLinks, clock, ids: new SeqIdGenerator() });
...
function seedProfile(h: Harness, overrides: Partial<ProfileRecord> & { id: string; userId: string }): ProfileRecord {
...
describe('ProfileService.getPublicProfileByHandle(実効公開ゲート)', () => {
  ...
  test('所有者が UNVERIFIED のとき第三者には NotFound で秘匿(AC-SHARE-008)', async () => {
    ...
    seedProfile(h, { id: 'p1', userId: 'u1', handle: 'minato' });
    await expect(h.service.getPublicProfileByHandle('minato')).rejects.toBeInstanceOf(NotFoundError);
  });
...
describe('ProfileService.updateProfileContent(検証・正規化)', () => {
  ...
  test('FROZEN は編集不可で Forbidden(BR-COMMON-005)', async () => {
    ...
    seedProfile(h, { id: 'p1', userId: 'u1' });
    await expect(
      h.service.updateProfileContent({ userId: 'u1', status: UserStatus.FROZEN }, { firstName: 'A' }),
...
describe('ProfileService.replaceSnsLinks', () => {
  test('11 件目は ValidationError(AC-PROF-015)', async () => {
    ...
    seedUser(h, 'u1', UserStatus.ACTIVE);
    seedProfile(h, { id: 'p1', userId: 'u1' });
    const links = Array.from({ length: 11 }, () => ({ platform: 'website', url: 'https://example.com' }));
  ...
  test('非 https は ValidationError(AC-PROF-014)', async () => {
    ...
    seedProfile(h, { id: 'p1', userId: 'u1' });
    await expect(
      h.service.replaceSnsLinks(activeViewer('u1'), [{ platform: 'github', url: 'http://github.com/x' }]),
    ).rejects.toBeInstanceOf(ValidationError);
  });
