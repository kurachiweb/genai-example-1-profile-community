...
describe('effectivePublic(BR-COMMON-007 実効公開ゲート)', () => {
	test('public かつ owner が ACTIVE のときのみ実効公開', () => {
		expect(effectivePublic({ visibility: Visibility.PUBLIC, ownerStatus: UserStatus.ACTIVE })).toBe(
			true
		);
	});
	...
	test('private のときは owner が ACTIVE でも実効非公開', () => {
		expect(
			effectivePublic({ visibility: Visibility.PRIVATE, ownerStatus: UserStatus.ACTIVE })
		).toBe(false);
	});
