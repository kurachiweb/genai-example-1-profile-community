...
describe('validateSnsLinks(BR-PROF-007)', () => {
	...
	test('javascript: スキームを拒否する(AC-PROF-014・XSS 防止)', () => {
		expect(() => validateSnsLinks([{ platform: 'website', url: 'javascript:alert(1)' }])).toThrow(
			ValidationError
		);
	});

	test('パースできない URL を拒否する', () => {
		expect(() => validateSnsLinks([{ platform: 'website', url: 'not a url' }])).toThrow(
			ValidationError
		);
	});
	...
	test('ラベル 31 文字は ValidationError(境界値)', () => {
		expect(() =>
			validateSnsLinks([
				{ platform: 'website', url: 'https://example.com', label: 'あ'.repeat(31) }
			])
