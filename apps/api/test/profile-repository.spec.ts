...
describe('MikroProfileRepository', () => {
  ...
  test('save は既存プロフィールのハンドルを更新する', async () => {
    ...
    await repo.save({ ...current!, handle: 'new-handle', updatedAt: new Date('2026-06-10T00:00:00.000Z') });
...
describe('MikroSnsLinkRepository', () => {
  ...
  test('replaceForProfile が全置換し findByProfileIds が sortOrder 昇順で返す', async () => {
    ...
    await repo.replaceForProfile('profile-a', [link('l1', 'profile-a', 0), link('l2', 'profile-a', 1)]);
    // 置換: 既存を消して 1 件に。
