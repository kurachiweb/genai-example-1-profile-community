...
- **`profiles.visibility` の格納値が `public`/`PUBLIC` のように大文字・小文字混在し、トップページや `/profiles` 一覧・公開切替の表示が不整合になる不具合を修正**（`apps/api`／`apps/public-api`）。実効公開ゲート（`BR-COMMON-007`）の正本である `Visibility` 定数が小文字（`'public'`/`'private'`）だった一方、`apps/client` は既に大文字（`'PUBLIC'`/`'PRIVATE'`）を前提に実装されていたため、両者の不一致でクライアント側の公開状態表示が常にずれていた。`Visibility` 定数を大文字へ統一し、既存データを正規化するマイグレーション（`Migration20260710120000`）を追加。公開 REST API（`apps/public-api`）は既存クライアントとの互換のため、書き込み時に小文字表記（`public`/`private`）も受理し大文字へ正規化する。
...
- **アカウント設定（`/settings`）でのメールアドレス変更確認リンクが 404 になる不具合を修正**（`apps/api`／`apps/client`）。`requestEmailChange`（`BR-ACCT-007`）は確認メール送信までしか実装されておらず、確認完了側の `UserService.verifyEmailChange`・GraphQL ミューテーション・client 側の確認画面（`/settings/confirm-email-change`）が欠落していた。確認完了時にメールアドレスを新アドレスへ確定し（`AC-ACCT-013`）、`status` を `ACTIVE`・`emailVerifiedAt` を現在日時に更新、乗っ取り対策として旧アドレスへ変更通知メールを送るよう `verifyEmailChange` を追加し、対応する確認画面を実装した。
