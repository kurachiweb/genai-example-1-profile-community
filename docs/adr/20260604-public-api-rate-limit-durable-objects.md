...
- セッション・ワンタイムトークン・短 TTL キャッシュ・公開API 以外のレート制限（認証系・通報系・一般閲覧）は既に **KV** で確定済み（[00-overview.md](../GUIDES/infra/00-overview.md) §3.1、[db/01-data-model.md](../GUIDES/db/01-data-model.md) §7）。
...
- **その他のレート制限カウンタ**（認証系・通報系・一般閲覧の各スコープ）と、**セッション・ワンタイムトークン・短 TTL キャッシュ**は引き続き **KV** に保存する（`rl:<scope>:<id>:<window>`）。
