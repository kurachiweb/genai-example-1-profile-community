...
| WebAuthn（パスキー） | 公開鍵暗号に基づく認証方式。認証器（端末内蔵・セキュリティキー）に秘密鍵を保持し、サーバーは公開鍵のみを保存する。利用者・管理者の双方で利用できる。1 アカウントに複数登録・命名・削除が可能。 | [00-common-rules.md](./features/00-common-rules.md) |
