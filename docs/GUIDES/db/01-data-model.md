...
| `search_name` | TEXT | nullable | 検索用の導出値。`first/last` ＋表示順を連結し NFC 正規化・ケースフォールド（`BR-DISC-004`/`BR-COMMON-009`、アプリ層で保守。→ [ADR 20260603](../../adr/20260603-profile-search-fts5.md)） |
| `bio` | TEXT | nullable | 最大 500 書記素・プレーンテキスト（`BR-PROF-006`）。検索対象（`BR-DISC-004`、保存時 NFC 正規化済み・クエリ時ケースフォールドで中間一致。専用インデックスなし） |
| `created_at` | datetime | NN | |
| `updated_at` | datetime | NN | |
...
> **氏名・職業・自己紹介検索（`BR-DISC-004`）**: **FTS5 は採用しない**（決定経緯は [ADR 20260603](../../adr/20260603-profile-search-fts5.md)）。表示名は `first/last` + 表示順から導出するため、検索用に NFC 正規化・ケースフォールド済みの結合名カラム `search_name` を持たせ、`idx_profiles_search_name` を張る。部分一致は `LIKE`、既定ソートの関連度は簡易ヒューリスティック（完全一致 ＞ 前方一致 ＞ 部分一致、氏名一致 ＞ 職業一致 ＞ 自己紹介一致）で実装する。職業・自己紹介（`bio`）は NFC 正規化済み（`BR-COMMON-009`）の値にクエリ時ケースフォールドして比較する。`bio` は語が文中に散在し前方一致が成立しにくいため専用インデックスを張らない。中間一致は B-tree インデックス非適用（全表スキャン依存）だが現規模では許容し、結果は短 TTL キャッシュ（`BR-DISC-006`）で補う。
