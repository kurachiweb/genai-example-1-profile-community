...
- **時刻は UTC で保存**し、表示時にローカルタイム（閲覧者の現地時間）へ変換する（`BR-COMMON-015`）。日時型は ISO-8601 文字列または整数エポックで一貫させる（MikroORM の `datetime`）。
...
- **大文字小文字非依存の一意性**（email・handle）:
  ...
  - `email` は表示用の原文と別に、小文字化・トリム済みの `email_normalized` を保持し、これに**ユニーク制約**を張る（`BR-ACCT-001`）。退会（`WITHDRAWN`）済み行は匿名化バッチ実行までメールアドレスが残るため対象外とする部分ユニークインデックスとする（[01-data-model.md](./01-data-model.md) §5.1/§6）。
