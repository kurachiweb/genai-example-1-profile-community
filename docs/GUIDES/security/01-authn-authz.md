...
- **ペッパー（サーバー側秘密鍵）による補完**: イテレーション数の上限（OWASP 推奨 600,000 に未達）を補うため、PBKDF2 の前段で `HMAC-SHA256(pepper, password)` を適用してから鍵導出する（`apps/api/src/infrastructure/password-hasher.ts`）。ペッパーは DB（ユーザーテーブル）とは物理的に独立した Cloudflare Workers Secrets（`PASSWORD_PEPPER`）に保管し、アプリケーションコード・リポジトリには含めない。この分離により、DB のみが漏洩したケースでは攻撃者はペッパーを持たずオフライン総当たり/辞書攻撃を開始できず、脅威モデル上「イテレーション数を上げて計算コストを上げる」よりも強い防御になる。ペッパーは32文字以上を必須とし、起動時（`loadEnv`）・ハッシュ化時の双方で検証する。現行実装はペッパーの自動ローテーション/デュアルペッパー検証を持たないため、ペッパー漏洩時は速やかに値を差し替えたうえで、影響範囲に応じて全ユーザーのパスワードリセットを強制する運用対応が必要になる（[operations/](../operations/) のインシデント対応手順に従う）。
